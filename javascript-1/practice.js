...
//return true as a boolean. If x does not equal true, return false as a boolean
...
// Create a function called 'oddChecker' that takes in one parameter, num. Check to see if the number is even or odd. If it
// is odd, return 'the number is odd' or return 'the number is even' if it is even.
...
// something you love. Have the function take the two parameters and retrun a string that says "NAMEPARAM loves LOVEPARAM" with the
...
//Code Here
function cutEnds(arr){
	arr.pop();
	arr.shift();
	return arr;
};

let middleNums = cutEnds(numbers);
...
function bigOrSmall(arr){
	let answers=[];
	 	arr.forEach(function(element){
			if(element>100){answers.push("big")}
...
function arrayReverser(arr){
	let reversed=[];
	for(i=arr.length-1;i>=0;i--){reversed.push(arr[i])}
	return reversed;
};
...
// Code Here
function greater100(value) {
	return value > 100;
  }

  let filtered = myNumbers.filter(greater100);
...
// Code Here
let total = myNumbers.reduce(myFunc);
...
// Code Here
let forTheLoveOfGeorge = notGeorge.map(function(value,i,arr){
	return "George"
	});
...
// Code Here
let enemies = people.filter(function(element,i,arr){return element.friend===false});
...
totallyAwesome=people.reduce(function(accumulator, currentValue, currentIndex, array) {
